...
void Channel::numFluxHLL_K(double q1m, double q1p, double q2m, double q2p, double *flux, bool Pm, bool Pp, int loc)                
{
	...
	double slow = 1e-5;
	//estimate speeds (Roe or fancier)
	speedsHLL_K(q1m, q1p, q2m, q2p, s, Pm, Pp, loc);
	// check for near zero speeds
	if (fabs(s[0])< slow && fabs(s[1])<slow) 
	{                                                       
		flux[0] = 0;
...
//	double dry = 1e-6*At;                                                  //pay attention to this!?
	...
	if(fmin(ym,yp)>=dry){
		...
		for(int i=1;i<Ns;i++){
			if (loc+2+i<=N){
				double Aplus = q_hist[idx_t(0,loc+2+i,n)];
				if (Aplus > maxA){maxA = Aplus;}
			}
			if (loc+1-i>=1){
				double Aminus = q_hist[idx_t(0,loc+2+i,n)];
		...
		s[0] = - findOmega(Astar, q1m, Ps, Pm);
		s[1] = findOmega(Astar, q1p, Ps, Pp);
...
double Junction3::findOmega(double Astar, double Ak, double D, double At, double Ts, bool Ps, bool Pk)
{
	double omega;
	double eps = 1e-8;
    if (Astar>Ak+eps)
		{
			omega = sqrt((Eta(Astar,D, At, Ts, Ps)-Eta(Ak,D, At,Ts, Pk))*Astar/(Ak*(Astar-Ak)));
		} 
	else
    	{
        	omega  = Cgrav(Ak,D, At, Ts,Pk);
    	}
	return omega;
}


double Junction3::getarea(double alpha, double old_alpha, double beta, double A_temp_past, double Q_temp_past, double Q0_past, double v2_past, double v0_past, double v0, double v1, double v2, double Q0, 
							double Q1, double Q2,double A0, double A1, double A2, double dt)
{
	//cout<<"get area from alpha"<<alpha<<endl;
	double A_temp;
	double A0_past = Q0_past/v0_past;
	/*
	double S_f;
	S_f = ch0.getFriction(A_temp_past,Q_temp_past);
	cout << "To calculate Sf: "<<A_temp_past<<" "<< Q_temp_past<<endl;
	cout << "Sf:"<<S_f<<endl;

	if (S_f<1e-8){    // initiallay it is 1e-2, then always copy A0
		if (A_temp_past == 0){cout<<"Very Small Sf, so copy A0 "<< A0<< endl;  return A0;} 
		else {cout<<"Very Small Sf, so copy A_temp_past, while A0 is "<< A0 << endl;  return (A_temp_past+A0)/2;}
	}
	*/
	double cal1; //This is L*dQ*(x)/dt
	cal1 = ch0.dx*(-1/2*pow((pow(v2/v0,2)+pow(beta,2)),-3/2)*(v2/v0-v2_past/v0_past)/dt*beta*Q0*alpha + (1+beta*v0/sqrt(pow(v2,2)+pow(beta*v0,2)))*((Q0-Q0_past)*alpha + Q0*(alpha-old_alpha))/dt);
	double cal2;
	cal2 = -cal1 + ch0.Eta(A0,false)-ch1.Eta(A1,false) + v0*Q0-v1*Q1-beta*v0*Q2 ;
	double Q_star = (1+beta*v0/sqrt(pow(v2,2)+pow(beta*v0,2))) * alpha * Q0;
	double rhs = G*pow(ch0.Mr,2)*pow(Q_star,2)*ch0.dx/cal2;
	if (rhs==0){cout<<"Zero RHS, A_temp = A0: "<<A0<<endl;return A0;}

	if (ch0.At*pow(ch0.w/4,4/3) <= rhs){ 
		cout<<"Whooo, A_temp > At"<<endl;
		A_temp = rhs/pow(ch0.w/4,4/3); }  
	else{
		cout<<"haha, A_temp < At"<<endl;
		double right_diff = get_diff(ch0.At, rhs);
		double left_diff = get_diff(0, rhs);
		A_temp = RecursiveArea(0, -rhs, ch0.At, right_diff, rhs);
	}

	cout<<"Related paremeters in cal_temp: "<<v2<<" "<< v0<<" "<< beta<<" "<< v2_past<<" "<< v0_past <<" "<< dt<<" "<< beta<< " "<< Q0<<" "<< alpha<< endl;
	cout<<"A0, A1, A2, Area: "<< A0 <<" "<<A1<<" "<<A2<< endl;
	//cout<<"cal_temp:" << cal_temp << endl;
	//A_temp = 1/(S_f*G)*( (ch0.Eta(A0,false)-ch1.Eta(A1,false))/ch0.dx - cal_temp + (v0*Q0-v1*Q1-beta*v0*Q2)/ch0.dx );
	cout<<"the calculated area is "<<A_temp<<endl;

	return A_temp;
}

/*It calculates A_temp in the above function recusively, since it is related to hydraulic radius*/
double Junction3::RecursiveArea(double leftA, double left_diff, double rightA, double right_diff, double rhs)
{
	double A = (leftA+rightA)/2;
	double diff = get_diff(A, rhs);
	if (abs(diff)<1e-5){return A;}
	if (diff*left_diff<0){return RecursiveArea(leftA, left_diff, A, diff, rhs);}
	if (diff*right_diff<0){return RecursiveArea(A, diff, rightA, right_diff, rhs);}
	else{cout<<endl<<"There must be something wrong with your RecursiveArea Function or related, and leftA, left_diff, A, diff, right A, right diff, rhs are:!!"
		<<" "<<leftA<<" "<<left_diff<<" "<<A<<" "<<diff<<" "<<rightA<<" "<<right_diff<<" "<<rhs<< endl<<endl;}

}

/*Help Function: It calculats the left and right hand side difference to help recursion*/
double Junction3::get_diff(double A, double rhs)
{
	double peri_water = ch0.getHydRad(A);
	double lhs = A*pow(peri_water,4/3);
	double re = lhs-rhs;
	return re;
}
...
/*
	It calculates the result of lhs-rhs
*/
double Junction3::getsigma(double alpha, double old_alpha, double beta, double A_temp_past, double Q_temp_past, double Q0_past, double v2_past, double v0_past, double v0, 
							double v1, double v2, double Q0, double Q1, double Q2,double A0, double A1, double A2, double dt)
{
	//cout<< "getSigma of alpha:"<<alpha<<endl;
	double A_temp = getarea(alpha, old_alpha, beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
	double Q_temp = alpha*Q0;
	double v_temp = Q_temp/A_temp;
	double cbar = (v0+v_temp)/2;
	double Astar = (A0+A_temp)/2.*(1+( (cbar>1e-6)? (v0-v_temp)/(2.*cbar): 0));
	double OL = findOmega(Astar, A0, ch0.w, ch0.At, ch0.Ts, false, false);
	double OR = findOmega(Astar, A_temp, ch0.w, ch0.At, ch0.Ts,false, false);
	double lhs = 2*Q_temp*Q0/A0 + Q0*A_temp/A0*OR-A_temp*OR*OL+v_temp*A0*OL+A0*OL*OR;
	double rhs = 2*v_temp*Q_temp+2*Q_temp*OR+2*Q_temp*OL;
	double sigma = lhs-rhs;
	//cout<<"sigma result "<<sigma<<endl;
	return sigma;
}

double Junction3::getalpha(double old_alpha, double left_alpha, double left_sigma, double right_alpha, double right_sigma, double alpha, 
							double beta, double A_temp_past, double Q_temp_past, double Q0_past, double v2_past, double v0_past, 
							double v0, double v1, double v2, double Q0, double Q1, double Q2,double A0, double A1, double A2, double dt)
{
	
	double sigma = getsigma(alpha, old_alpha, beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
	double new_alpha;
	//cout << "left alpha, sigma: "<< left_alpha<<", "<<left_sigma<<endl;
	//cout << "right alpha, sigma: "<< right_alpha<<", "<<right_sigma<<endl;
	//cout << "new alpha, sigma: "<< alpha<<", "<<sigma<<endl;
	if (fabs(sigma) < 1e-5){cout<<"Yes, get ALPHA!!!: "<<alpha<<endl;return alpha;}
	if (sigma*left_sigma < 0){
		new_alpha = (alpha + left_alpha)/2;
		return getalpha(old_alpha, left_alpha, left_sigma, alpha, sigma, new_alpha,  
			beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
	}
	else if (sigma*right_sigma<0){
		new_alpha = (alpha+right_alpha)/2;
		return getalpha(old_alpha, alpha, sigma, right_alpha, right_sigma, new_alpha,      
			beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
	}
		else{
			cout << "Both left and right is large than zero! Please make sure you know the boundary!!"<<endl;
		}
}
...
void Junction3::boundaryFluxes(double dt){
	...
	if(whichend[0]==1 &&whichend[1] ==0 &&whichend[2] ==0)
	{
		...
		*/
		...
		double A_temp_past, Qx_temp_past, Qy_temp_past;
		A_temp_past = ch0.q_hist[ch0.idx_t(0,ch0.N+1,ch0.n-1)];
		Qx_temp_past = ch1.q_hist[ch1.idx_t(1,0,ch1.n-1)];
		Qy_temp_past = ch2.q_hist[ch2.idx_t(1,0,ch2.n-1)];
		double vx_temp_past = (A_temp_past>1e-5 ?Qx_temp_past/A_temp_past:0.);


		//time step n
		double A0, Q0, A1, Q1, A2, Q2; 
		double A_temp, Qx_temp, Qy_temp;
		...
		*****************************************d\****************/
		...
		//double Q_past = sqrt(pow(Qx_temp_past,2)+pow(Qy_temp_past,2));
		double Q_past = Qx_temp_past + Qy_temp_past;
		//double Q_past = Qx_temp_past;

		//ch0.numFluxHLL1(A0_past,A_temp_past, Q0_past, Qx_temp_past, ch0.bfluxright, true, true, Qy_temp_past);
		ch0.numFlux(A0_past,A_temp_past, Q0_past, Q_past, ch0.bfluxright, true, true);
		ch1.numFlux(A_temp_past,A1_past, Qx_temp_past -0*beta*v0_past*Qy_temp_past*A2_past/Q2_past, Q1_past, ch1.bfluxleft, true, true);
		ch2.numFlux(A_temp_past,A2_past, Qy_temp_past, Q2_past, ch2.bfluxleft, true, true);
		double Qin = ch0.bfluxright[0];
		double Qout1 = ch1.bfluxleft[0];
		double Qout2 = ch2.bfluxleft[0];
		//double L = ch2.w;
		double L = ch0.dx;
		A_temp = (Qin - Qout1 - Qout2)*dt/L +A_temp_past;
		cout<<"get A_temp: "<<A_temp<<endl;
		//double A0_middle = (A0_past + A_temp_past)/2*(1+(v0_past-vx_temp_past)/(v0_past + vx_temp_past));
		//double A_temp_middle = (A1_past + A_temp_past)/2*(1+(vx_temp_past-v1_past)/(v1_past + vx_temp_past));
		Qx_temp = (ch0.Eta(A0_past, true) - ch1.Eta(A1_past, true) + pow(Qin,2)/A0_past - pow(Qout1,2)/A_temp_past - Qout2*beta*v0_past)*dt/L + Qx_temp_past;
		cout << "get Qx_temp:"<< Qx_temp<<endl;
		double Qy_temp1 = (pow(Q0,2)/2/G/pow(A0,2) + ch0.Eta(A0,true)/G/A0 - ch0.Eta(A_temp, true)/G/A_temp)*2*G*pow(A_temp,2)-pow(Qx_temp,2); 
		if (Qy_temp1<0){
			Qy_temp = 0;
		}
		else{
			Qy_temp = sqrt(fabs(Qy_temp1));
	    }
		//double Q_try= sqrt(-Qy_temp1);
		cout << "get Qy_temp1, Qy_temp: "<<Qy_temp1 << ", "<< Qy_temp << endl;
		}
		
		// Calculate Boundary Flux
		//double Q_now = sqrt(pow(Qx_temp,2)+pow(Qy_temp,2));
		double Q_now = Qx_temp + Qy_temp;
		//double Q_now = Qx_temp;

		//ch0.numFluxHLL1(A0,A_temp, Q0, Qx_temp, ch0.bfluxright, true, true, Qy_temp);
		ch0.numFlux(A0,A_temp, Q0, Q_now, ch0.bfluxright, true, true);
		ch1.numFlux(A_temp,A1, Qx_temp - 0*beta*v0*Qy_temp*A2/Q2, Q1, ch1.bfluxleft, true, true);
		ch2.numFlux(A_temp,A2, Qy_temp, Q2, ch2.bfluxleft, true, true);

		// Store junction value 
		ch0.q_hist[ch0.idx_t(0,ch0.N+1,ch0.n)] = A_temp; 
		ch1.q_hist[ch1.idx_t(1,0,ch1.n)] =  Qx_temp;
		ch2.q_hist[ch2.idx_t(1,0,ch2.n)] =  Qy_temp;
...
/*
		double c0_fake = v0 - ch0.Cgrav(A0,false);

		if (v0_past ==0 || v0== 0){
			ch0.q_hist[ch0.idx_t(0,ch0.N+1,ch0.n)] = A_temp_past; 
			ch0.q_hist[ch0.idx_t(1,ch0.N+1,ch0.n)] = Q_temp_past;
			ch1.q_hist[ch1.idx_t(0,0,ch1.n)] =  A_temp_past;
			ch1.q_hist[ch1.idx_t(1,0,ch1.n)] =  Q_temp_past;
			ch2.q_hist[ch2.idx_t(0,0,ch2.n)] =  A_temp_past;
			ch2.q_hist[ch2.idx_t(1,0,ch2.n)] =  Q_temp_past;
			ch0.numFlux(A0,A_temp_past, Q0, Q_temp_past, ch0.bfluxright, true, true);
			ch1.numFlux(A_temp_past,A1, Q_temp_past, Q1, ch1.bfluxleft, true, true);
			double frac;
			if (v2==0){frac =1;}
			else {frac = v2/sqrt(pow(v2,2)+pow(beta*v0,2));}
			ch2.numFlux(A_temp_past,A2, frac*Q_temp_past, Q2, ch2.bfluxleft, true, true);
			return;
		}

		double frac = v2/sqrt(pow(v2,2)+pow(beta*v0,2));
		double s0[2]={0,0};
		double s1[2]={0,0};
		double s2[2]={0,0};

		cout << " Non-zero velocity, so v0, v0_past:"<<v0<<" "<<v0_past<<endl;
		if (c0_fake >0){
			cout<<"situation 1!!"<<endl;
			Q_temp = Q0/2;
			A_temp = getarea(1/2, 1/2, beta,A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
			ch0.old_alpha = 1/2;
			// Check if the situation is right divided
			ch0.speeds(A0, A_temp, Q0, Q_temp, s0, true, true);
			if (s0[0]>0){cout<<"from channel 0 to ghost, sL>0"<<endl;}
			if (s0[0]<=0 && s0[1]>=0){cout<<"from channel 0 to ghost, sL<=0<=sR"<<endl;}
			if (s0[1]<0){cout<<"from channel 0 to ghost, sR<0"<<endl;}
			ch1.speeds(A_temp, A1, Q_temp, Q1, s1, true, true);
			if (s1[0]>0){cout<<"from ghost to channel 1, sL>0"<<endl;}
			if (s1[0]<=0 && s1[1]>=0){cout<<"from ghost to channel 1, sL<=0<=sR"<<endl;}
			if (s1[1]<0){cout<<"from ghost to channel 1, sR<0"<<endl;}
			ch1.speeds(A_temp, A2, frac*Q_temp, Q2, s2, true, true);
			if (s2[0]>0){cout<<"from ghost to channel 2, sL>0"<<endl;}
			if (s2[0]<=0 && s2[1]>=0){cout<<"from ghost to channel 2, sL<=0<=sR"<<endl;}
			if (s2[1]<0){cout<<"from ghost to channel 2, sR<0"<<endl;}
		}
		else{
			cout<<"situation 2!!"<<endl;
			double zero_sigma = getsigma(0, ch0.old_alpha, beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
			double one_sigma = getsigma(1, ch0.old_alpha, beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
			if (zero_sigma*one_sigma>0){
				cout<<"0 times 1 is large than 0, what happened??"<<endl;
			}
			double alpha = getalpha(ch0.old_alpha, 0, zero_sigma, 1, one_sigma, 0.5, 
						beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2, A0, A1, A2, dt);
			Q_temp = alpha*Q0;
			A_temp = getarea(alpha, ch0.old_alpha, beta, A_temp_past, Q_temp_past, Q0_past, v2_past, v0_past, v0, v1, v2, Q0, Q1, Q2,A0, A1, A2, dt);
			ch0.old_alpha = alpha;
			// Check if the situation is rightly divided, same as above
			ch0.speeds(A0, A_temp, Q0, Q_temp, s0, true, true);
			if (s0[0]>0){cout<<"from channel 0 to ghost, sL>0"<<endl;}
			if (s0[0]<=0 && s0[1]>=0){cout<<"from channel 0 to ghost, sL<=0<=sR"<<endl;}
			if (s0[1]<0){cout<<"from channel 0 to ghost, sR<0"<<endl;}
...
*/
		// store the sharing ghost cell values 
/*		ch0.q_hist[ch0.idx_t(0,ch0.N+1,ch0.n)] = A_temp; 
		ch0.q_hist[ch0.idx_t(1,ch0.N+1,ch0.n)] = Q_temp;
		ch1.q_hist[ch1.idx_t(0,0,ch1.n)] =  A_temp;
		ch1.q_hist[ch1.idx_t(1,0,ch1.n)] =  Q_temp;
		ch2.q_hist[ch2.idx_t(0,0,ch2.n)] =  A_temp;
		ch2.q_hist[ch2.idx_t(1,0,ch2.n)] =  Q_temp;
*/		
		// Calculate boundary flux with (A_temp,Q_temp)
/*
		ch0.numFlux(A0,A_temp, Q0, Q_temp, ch0.bfluxright, true, true);
		ch1.numFlux(A_temp,A1, Q_temp, Q1, ch1.bfluxleft, true, true);

		ch2.numFlux(A_temp,A2, frac*Q_temp, Q2, ch2.bfluxleft, true, true);

*/

		//****************************************************
		//My own algorithm ends
...
/*
	...
	//0-1 is easy
		...
		cout<<"the 0-1 subproblem result F(0,1):"<< ch1.bfluxleft[0] <<", "<< ch1.bfluxleft[1]<<endl; 
	...
	//1-2 is a pain in the neck	
		...
		cout<<"the 1-2 subproblem result F(0,1):"<< ch1.bfluxleft[0] <<", "<< ch1.bfluxleft[1]<<endl; 
		...
		flux2[1] = p21*ch2.bfluxleft[1];
		cout<<"the 2-1 subproblem result F(0,1):"<< ch2.bfluxleft[0] <<", "<< ch2.bfluxleft[1]<<endl; 
		...
		cout<<"the 2-0 subproblem result F(0,1):"<< ch2.bfluxleft[0] <<", "<< ch2.bfluxleft[1]<<endl;
...
*/
		...
		//store away the info
		//cout<<"[After_A]0:"<<Abar[0]<<" 1:"<<Abar[1]<<" 2:"<<Abar[2]<<endl;
		//cout<<"[After_Q]0:"<<Qbar[0]<<" 1:"<<Qbar[1]<<" 2:"<<Qbar[2]<<endl;
		//cout<<"the stored location of channle 0 in qhist:"<< ch0.idx_t(0,ch0.N+1,ch0.n)<<endl;
		//cout<<"the stored location of channle 1 in qhist:"<< ch1.idx_t(0,0,ch1.n)<<endl;
		//cout<<"the stored location of channle 2 in qhist:"<< ch2.idx_t(0,0,ch2.n)<<endl;

	}
    else if( (whichend[0] ==0&& whichend[1] ==1 &&whichend[2] ==1))
	{
		cout<<'can also be here'<<endl;
